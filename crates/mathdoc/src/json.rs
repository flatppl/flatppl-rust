...
pub fn render_math(input: &str) -> Result<String, String> {
    ...
    let want_mathml = req.formats.iter().any(|f| f == "mathml");
...
mod tests {
    ...
    fn the_contract_round_trips() {
        ...
        assert_eq!(v["order"], serde_json::json!(["mu", "a"]));
        ...
        assert_eq!(v["bindings"][1]["names"], serde_json::json!(["a", "b"]));
        assert_eq!(v["bindings"][0]["refs"], serde_json::json!(["mu"]));
