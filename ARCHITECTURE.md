...
**Status: living architecture reference.** Records the design decisions for the
`flatppl-rust` toolchain as they are made; the frontend and tooling crates listed
under "Crates" below have landed, while the Phase-3+ codegen/backend surface stays
forward-looking design. For the language semantics, see the FlatPPL spec (`flatppl-design`).
...
- **Nodes:** a small enum (`lit`/`const`/`ref`/`hole`/`axis`/`call`). Literals are a
  typed `Scalar` enum (`Int`/`Real`/`Bool`/`Str`); complex numbers, vectors and
  records are constructor calls, not literals. Nested array types are
...
the target dialect"). The **determiniser** (crate `flatppl-determinizer`, landed) =
*"legalize to the FlatPDL profile"* — the first such driver, a greedy legalizer over
its rule set (currently in-crate; extracting a catalog *shared* with other target
drivers is forward work), not a bespoke monolith. Other targets become other drivers
...
own cargo feature (`convert`/`infer`/`prepare`/`hs3` all enabling `flatpir`;
`determinize` pulls `flatppl-determinizer` instead). Lint rules:
...
extracting a *shared* multi-target lowering catalog (possibly merged with a
...
- Lowering **driver flavour** (ordered pipelines vs automatic vs egglog): the
  greedy scan-and-apply legalizer is chosen and landed for the directional FlatPDL
  path; egglog for the cross-paradigm/raising path stays open.
